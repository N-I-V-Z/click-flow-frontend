export default class GET {
  ...
  */
  ...
  /**
   * Check độ phức tạp của password trả về strength
   * @param _password
   */
  ...
  /**
   * Check độ phức tạp của password trả về message
  ...
  getRandomHash = (XLength: number) => {
    ...
    // Default is 10;
    const Length = XLength ? parseInt(`` + XLength) : 10;
  ...
  getRandomNumber = (string_length = 10): string => {
    let text = '';
    // Default is 10;
    const Length = string_length ? Number(string_length) : 10;
  ...
  /**
   * Cắt content ra thành độ dài mong muốn, thêm dấu 3 chấm
   * @McJamBi  from                     Jamviet.com
  ...
  getNonUnicode = (_string: string, special?: boolean) => {
    let str = String(_string || ' ').trim();
    if (str === '') return '';

    const unicode_char: any = {
    ...
    for (var name in unicode_char) {
      let value = unicode_char[name].split('|');
   ...
   */
  slugify = (_string: any) => {
    const ss = this.unicodeToChar(_string);
