...
export default class helper_parse {
  ...
  parseInt(nnn: any, defaultx?: any): number {
    const n = String(nnn || ' ').match(/\d+/gim);
  ...
  parseNumeric(nnn: any, defaultx?: any): number {
    ...
    const n = String(nnn || ' ').replace(/[^\d-]+/g, '');
   ...
   */
  parseChar(nnn: any) {
    const n = String(nnn || ' ').replace(/[^A-Za-z0-9]/g, '');
    if (!n) return '';
  ...
  parseAnphabet(_string: any) {
    const n = String(_string || ' ').replace(/[^\w!.?:]/g, '');
    if (!n) return '';
    return String(n).trim();
  }
  ...
  parseURL(_url: any) {
    try {
      const n = String(_url || ' ').replace(/[^A-Za-z0-9._\-/#&!;]/g, '-');
      if (!n) return '';
  ...
  parseEmailList(emailList: any) {
    ...
    const s = String(emailList || ' ').split(',');
